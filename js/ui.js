...
function toggleLoading(isLoading) {
  ...
  const searchbtn = document.getElementById("searchbutton"); //button itself
  const searchicon = document.getElementById("search_icon");    // Icon
  const btntext = document.getElementById("searchtext");     // Search

  if (isLoading) {
    searchicon.classList.remove("fa-search");
    searchicon.classList.add("fa-hourglass-half");
    btntext.textContent= "Loading...."
    searchbtn.disabled= true;
  }

    if (!isLoading) {
      searchicon.classList.remove("fa-hourglass-half");
